...
Proyek Supabase Tamuara terhubung ke repository GitHub, dengan deploy production otomatis tetap nonaktif. Enam migrasi sudah diterapkan ke proyek tersebut. Smoke test lokal dan online dengan tiga akun sintetis telah meluluskan Auth, draf/audit, isolasi pasangan, media privat WebP, pratinjau dan pencabutan token, serta antrean/klaim bantuan oleh admin dengan entitlement paket bantuan. Media privat dibaca melalui endpoint aplikasi; akses unduh langsung dari Storage ditutup. Penurunan peran admin segera mencabut aksesnya ke undangan dan media. Pesanan berstatus lunas dalam tes hanyalah fixture sintetis; semua data uji dibersihkan dan diverifikasi. Proyek ini dipakai sementara sebagai lingkungan uji. Lihat [panduan Supabase](supabase/README.md) untuk menjalankan ulang tes. Site URL Auth dan dua URL callback sudah disesuaikan dengan `http://127.0.0.1:3001`; SMTP dan pengujian email nyata masih diperlukan. Akun merchant belum tersedia; checkout Midtrans dan webhook belum terhubung. Mode Supabase menolak checkout (503) dan pembayaran simulasi sehingga aktivasi paket sungguhan belum tersedia.
...
Untuk pengujian provider Supabase pada mesin lokal, jalankan `npx.cmd supabase start`, `npx.cmd supabase test db --local`, lalu `npm.cmd run test:supabase:local` di PowerShell. Enam migrasi telah lulus **90 pemeriksaan SQL**. Smoke test lokal dan online menguji media privat melalui aplikasi, penolakan unduh Storage langsung, rotasi token pratinjau, antrean/klaim admin, dan pencabutan akses saat admin diturunkan perannya, dengan pembersihan data sintetis.

Pengujian backend menggunakan direktori sementara dan klien Supabase tiruan untuk memeriksa isolasi pelanggan, pembayaran simulasi, konflik versi, publikasi, token, kuota RSVP, moderasi, akses media, serta pemeriksaan asal request. Pengujian browser menjalankan alur pasangan–tamu dan admin, impor CSV, serta layout desktop/ponsel di Chromium. **25 tes backend/HTTP** dan 5 tes browser telah lulus.

Playwright menjalankan server demo lokal terpisah pada port 3002 (dapat diubah dengan `TAMUARA_E2E_PORT`), memakai `.data/playwright` dan `.next-e2e`. Tes tidak memakai ulang server port 3001 yang mungkin sedang terhubung ke Supabase. Browser tests membuat akun sintetis pada data demo lokal; tidak mengirim email atau pesan WhatsApp. Screenshot berada di `test-results/` dan tidak masuk Git. Enam migrasi dan 90 pemeriksaan pgTAP telah lulus pada Supabase lokal. Alur Auth, media, pratinjau, permintaan/klaim bantuan admin dengan pesanan sintetis berstatus lunas, serta pencabutan akses admin telah lulus pada proyek Supabase online; Safari/iOS fisik, email nyata, publikasi, dan pembayaran pelanggan belum diuji end-to-end di sana.
