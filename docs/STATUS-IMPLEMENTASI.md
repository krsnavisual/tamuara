...
| Database | Tiga migrasi diterapkan ke Supabase lokal; dua berkas pgTAP lulus dengan 49 pemeriksaan. Migrasi belum diterapkan ke proyek online |
| Provider lokal | Dua akun Auth sintetis menghasilkan profil owner; login route, pembuatan draf, audit, dan isolasi antar pasangan lulus melalui Supabase lokal |
| Pembayaran | Checkout produksi dinonaktifkan sampai merchant dan webhook tersedia; simulasi hanya di mode lokal |

Adapter Supabase belum dianggap siap peluncuran. Proyek online belum dikonfigurasi dengan kredensial aplikasi atau migrasi. Organisasi Supabase telah mencapai batas proyek gratis sehingga staging online terpisah belum tersedia. Seluruh data undangan di mode Supabase saat ini disimpan sebagai satu dokumen per undangan; untuk volume RSVP tinggi, pecah transaksi ke tabel terstruktur yang sudah disiapkan.
...
5. Pengelolaan akun/penugasan dan audit viewer untuk pengelola platform. Admin lokal mempunyai penugasan contoh; belum ada panel superadmin penuh.
...
Hasil tahap ini: **22 tes backend/HTTP, 49 pemeriksaan SQL pgTAP dan smoke test provider pada Supabase lokal, 5 tes browser Chromium, lint, TypeScript, dan build lulus**. Adapter Supabase dengan proyek online, alur email, dan pembayaran masih perlu diuji.
